...
let UsersService = class UsersService {
    ...
    async login(loginDto) {
        try {
            const userExists = await this.findUserByEmail(loginDto.email);
            if (!userExists) {
                return constant_1.LOGIN_MSG.NOT_EXIST;
            }
            const isPasswordValid = await bcrypt.compare(loginDto.password, userExists.password);
            if (!isPasswordValid) {
                return constant_1.LOGIN_MSG.NOT_MATCH;
            }
            const userToken = await (0, jsonwebtoken_1.sign)({
                userId: userExists._id,
                email: userExists.email,
                role: userExists.userRole,
            }, "BookStore", { expiresIn: "1h" });
